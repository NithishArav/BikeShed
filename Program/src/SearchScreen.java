...
public class SearchScreen extends JPanel implements Screen
{
    ...
    public SearchScreen(View view, ActionListener mainListener, ActionListener paperListener)
    {
        ...
        searchParam = new JComboBox<String>(searchItems.keySet().toArray(new String[searchItems.size()]));
        add(searchParam, v.getConstraint(0, 3));

        add(new JLabel("Sort by: "), v.getConstraint(1, 2));
        sortParam = new JComboBox<String>(comparableItems.keySet().toArray(new String[comparableItems.size()]));
        sortInput = new JComboBox<String>(sortItems.get(STR));
        sortParam.addActionListener(new ActionListener() {
            @Override
        ...
        filterParam.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                JComboBox<String> cb = (JComboBox<String>)e.getSource();
                String selectedItem = (String)cb.getSelectedItem();
                filterInput.setModel(new DefaultComboBoxModel<String>(filterItems.get(comparableItems.get(selectedItem))));
            }
        });
